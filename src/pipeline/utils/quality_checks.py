...
from pyspark.sql.types import DecimalType, DateType, DatetimeType
...
shared_columns = {
    ...
    'unit_cost_usd': pa.Column(DecimalType(12, 2), checks=pa.Check.greater_than_or_equal_to(0)),
}
...
inventory_schema = pa.DataFrameSchema(
    {
        ...
        'reorder_point': pa.Column(int, checks=pa.Check.greater_than(0)),
...
suppliers_schema = pa.DataFrameSchema({
    ...
    #'order_date': pa.Column(DatetimeType, nullable=True),
    'supplier_name': shared_columns['supplier_name'],
    'category': shared_columns['category'],
    'sku': shared_columns['sku'],
    'product_name': shared_columns['product_name'],
    ...
    'qty_received': pa.Column(int, pa.Check.greater_than(0), nullable=True),
    'unit_cost_usd': shared_columns['unit_cost_usd'],
    'total_order_value_usd': shared_columns['unit_cost_usd'],
    'expected_delivery_date': pa.Column(DateType()),
    #'actual_delivery_date': pa.Column(DatetimeType, nullable=True),
...
transactions_schema = pa.DataFrameSchema({
    ...
    'customer_id': pa.Column(str, pa.Check.str_matches(r"CUST-[0-9]{4}")),
    #'transaction_date': pa.Column(DatetimeType, nullable=True),
